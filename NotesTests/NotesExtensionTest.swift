...
//  Edited by Paul Vasilenko on 6/20/19.
...
class NoteExtensionsTests: XCTestCase {
    ...
    func testNoteExtensions_whenGetJsonWithoutDate_hasNotSaveDate() {
        let note = Note(title: "Text", content: "More", importance: .important, selfDestructionDate: Date())
        let json = note.json
    ...
    func testNoteExtensions_whenGetJsonAndParseJsonForFullNote_isEqualsNotes() {
        let originNote = Note(uid: "1234", title: "Title1", content: "My text", color: .red, importance: .unimportant, selfDestructionDate: Date())
